...
const trashColor = ref('rgb(187, 187, 187)')
const isHovered = ref(false)

const hoverChangeColor = (hovered) => {
  isHovered.value = hovered
  trashColor.value = hovered ? '#878787' : 'rgb(187, 187, 187)'
}

const clickChangeColor = () => {
  trashColor.value = 'black'

  setTimeout(() => {
    trashColor.value = '#878787'
  }, 125)
}
</script>
...
<template>
  <button class="select-none cursor-pointer">
    <svg
      ...
      v-on:mouseover="hoverChangeColor(true)"
      v-on:mouseout="hoverChangeColor(false)"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
